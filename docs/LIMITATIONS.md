...
- **Storage on SQLite: ~444 bytes/quad [SQLite-only]** measured on the SlideDek port-benchmark corpus (133M quads / 60 GB / ~140 distinct predicates / 60K+ PPTX decks — exercised during the ArangoDB → quadstore port effort, not a live deployment). Subjects, predicates, objects, and labels are all stored as `TEXT` verbatim. No string interning, no predicate dictionary, no value compression. **Pebble at production scale: ~10× smaller** — a 19M-quad / 28 GB SecDek SQLite snapshot round-tripped to ~3 GB Pebble dir after default zstd block compression.
