...
- **Single-quad `Writer.Commit`: ~112 µs.** Most of that is the `commits` + `commit_ops` audit row writes — by design. If you need higher single-commit throughput, batch your `Adds` into a single `Commit` call instead.
...
- **No `Batch.NoAudit` opt-out on `Writer.Commit`.** The `commits` + `commit_ops` audit rows are the right default and the reason single-quad commits are ~4.5× slower than the same write in raw SQLite. Workloads that don't need the audit trail must currently use `BulkLoader`, which has different ergonomics. A clean opt-out is on the table; the API shape isn't decided.
